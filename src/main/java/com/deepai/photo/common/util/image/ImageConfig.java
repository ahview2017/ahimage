...
public class ImageConfig {
     ...
     * @throws Exception
     */
    public static String getFilePath(String filename, String size, int siteId,
            SysConfigService sysConfigService) throws Exception {
        return getFilePathOnServer(getPictureSizeType(size), filename, siteId,
                sysConfigService);
    }
    ...
    public static String getFilePathOnServer(ArchivePictureSize aps,
            String filename, int siteId, SysConfigService sysConfigService)
            throws Exception {
        StringBuffer sb = new StringBuffer();
        if (StringUtils.isNotBlank(filename)) {
            switch (aps) {
            case SMALL:
                sb.append(sysConfigService.getDbSysConfig(
                        SysConfigConstant.SMALL_PIC_PATH, siteId));
                break;
            case MEDIUM:
                sb.append(sysConfigService.getDbSysConfig(
                        SysConfigConstant.MEDIUM_PIC_PATH, siteId));
                break;
            ...
            case BIG:
                sb.append(sysConfigService.getDbSysConfig(
                        SysConfigConstant.BIG_PIC_PATH, siteId));
                break;
            case ORIGINAL:
                sb.append(sysConfigService.getDbSysConfig(
                        SysConfigConstant.ORIGINAL_PIC_PATH, siteId));
                break;
            default:
                sb.append(sysConfigService.getDbSysConfig(
                        SysConfigConstant.SMALL_PIC_PATH, siteId));
            ...
            sb.append(sysConfigService
                    .getDbSysConfig(SysConfigConstant.SMALL_PIC_PATH, siteId));
            sb.append(File.separator);
            sb.append(sysConfigService
                    .getDbSysConfig(SysConfigConstant.DEFAULT_PICTURE, siteId));
        }
     ...
     * @throws Exception
     */
    public static String getCoverPicturePathOnServer(String filename,
            int siteId, SysConfigService sysConfigService) throws Exception {
        StringBuffer sb = new StringBuffer();
        if (StringUtils.isNotBlank(filename)) {
            sb.append(sysConfigService
                    .getDbSysConfig(SysConfigConstant.COVER_PIC_PATH, siteId));
            // 无论配置中的路径后是否有路径分隔符，都在路径后再加一个分隔符
            sb.append(File.separator);
            sb.append(filename);
        } else {
            ...
            sb.append(File.separator);
            sb.append(sysConfigService.getDbSysConfig(
                    SysConfigConstant.DEFAULT_COVER_PICTURE, siteId));
        }
        ImgFileUtils.makeDirectory(sb.toString());
        return sb.toString();
    }
     ...
     * @throws Exception
     */
    public static String getWaterMarkPicturePathOnServer(String filename,
            int siteId, SysConfigService sysConfigService) throws Exception {
        StringBuffer sb = new StringBuffer();
        if (StringUtils.isNotBlank(filename)) {
            sb.append(sysConfigService.getDbSysConfig(
                    SysConfigConstant.DEFAULT_WATERMARK_PATH, siteId));
     ...
     * @throws Exception
     */
    public static String getTempPathOnServer(String filename, int siteId,
            SysConfigService sysConfigService) throws Exception {
        StringBuffer sb = new StringBuffer();
        if (StringUtils.isNotBlank(filename)) {
            sb.append(sysConfigService
     ...
     * @throws Exception
     */
    public static String getPicSizeOnServer(String filename, int siteId,
            SysConfigService sysConfigService) throws Exception {
        StringBuffer sb = new StringBuffer();
        if (StringUtils.isNotBlank(filename)
                && "1000".equalsIgnoreCase(filename)) {
            sb.append(sysConfigService
                    .getDbSysConfig(SysConfigConstant.PICSIZE_1000, siteId));
        }
        if (StringUtils.isNotBlank(filename)
                && "2000".equalsIgnoreCase(filename)) {
            sb.append(sysConfigService
                    .getDbSysConfig(SysConfigConstant.PICSIZE_2000, siteId));
        }
        if (StringUtils.isNotBlank(filename)
                && "3000".equalsIgnoreCase(filename)) {
            sb.append(sysConfigService
                    .getDbSysConfig(SysConfigConstant.PICSIZE_3000, siteId));
        }
        ImgFileUtils.makeDirectory(sb.toString());
        return sb.toString();
    }
