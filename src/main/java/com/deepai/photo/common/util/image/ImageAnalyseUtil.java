...
public class ImageAnalyseUtil {
    private static Logger logger = LoggerFactory
            .getLogger(ImageAnalyseUtil.class);

    public static enum Color {
        red(255 * 256 * 256 + 0 * 256 + 0, 0), orange(
                255 * 256 * 256 + 128 * 256 + 0,
                1), yellow(255 * 256 * 256 + 255 * 256 + 0, 2), green(
                        0 * 256 * 256 + 255 * 256 + 0,
                        3), cyan(0 * 256 * 256 + 255 * 256 + 255,
                                4), blue(0 * 256 * 256 + 0 * 256 + 255,
                                        5), purple(
                                                128 * 256 * 256 + 0 * 256 + 255,
                                                6), white(255 * 256 * 256
                                                        + 255 * 256 + 255,
                                                        7), black(0, 8);
     ...
     * @throws Exception
     */
    private static Metadata readMetadataFromFile(String filePath)
            throws Exception {

        File imgFile = new File(filePath);
        if (filePath.toLowerCase().lastIndexOf("tif") >= filePath.length()
                - 4) {
     ...
     * @throws Exception
     */
    public static Directory extratIPTCFromFile(String filePath)
            throws Exception {
     ...
     * @throws Exception
     */
    public static Directory extratEXIFFromFile(String filePath)
            throws Exception {
     ...
     * @throws Exception
     */
    public static void alterPic(int size, File file, String Path_new,
     ...
     * @throws Exception
     */
    public static void gmAlterImg(int size, String srcPath, String destPath,
            int width, int height, Boolean synFlag) throws Exception {
        try {
            //
...
//            convert.setSearchPath("/usr/local/graphicsmagick/bin");
    ...
    public static int getSize(File file, BufferedImage src) throws Exception {
        ...
        try {
            if (file.getName().substring(file.getName().lastIndexOf("."))
                ...
                return rImage.getWidth() > rImage.getHeight()
            ...
            int size = src.getWidth(null) > src.getHeight(null)
     ...
     * @throws Exception
     */
    private static void alterSize(int new_w, int new_h, File file,
            String Path_new) throws Exception {
        if (!file.exists()) {
            throw new Exception("big pic not exists");
        }
        if (file.getName().substring(file.getName().lastIndexOf("."))
     ...
     * @throws Exception
     */
    public static BufferedImage getGraphics(String file_path, int siteId,
        ...
        try {
            if (file.getName().substring(file.getName().lastIndexOf("."))
                ...
                File tempFile = new File(sysConfigService
                        .getDbSysConfig(SysConfigConstant.TEMP_PIC_PATH, siteId)
                ...
                ImageDecoder dec = PNGCodec.createImageDecoder("png", s,
                        pngParam);
                RenderedImage rImage = dec.decodeAsRenderedImage();
     ...
     *             add by dulei 20120717
     */
    public static int[] getWatermarkPosition(int b_width, int b_height,
            int w_width, int w_height, int position) {
        int[] xy = new int[2];
        switch (position) {
        case 1:
            xy[0] = 0;
     ...
     * @throws Exception
     */
    public static BufferedImage handlePic(BufferedImage basepicture,
            BufferedImage waterMark, HttpServletRequest req,
            HttpServletResponse res, SysConfigService sysConfigService)
            throws Exception {
        int siteId = SessionUtils.getSiteId(req);
        String waterPosition = sysConfigService
                .getDbSysConfig("ORIGINAL_PIC_PATH", siteId);
        String picSize = req.getParameter("picSize");
        int picRealSize = Integer.parseInt(ImageConfig
                .getPicSizeOnServer(picSize, siteId, sysConfigService));
        int posW = 0;
        int posH = 0;
        ...
        if ("1000".equalsIgnoreCase(picSize)) {
            basepicture = ImageAnalyseUtil.alterSize(basepicture, bigPicWidth,
                    bigPicHeight);
        }
     ...
     */
    public static BufferedImage markPicture(BufferedImage basePicture,
            BufferedImage waterMark, int position_x, int position_y,
            int DarkPercent, boolean ignoreWhite, String waterPicType) {
        int w_width = waterMark.getWidth();
        int w_height = waterMark.getHeight();
        int[] b_pixels = basePicture.getRGB(position_x, position_y, w_width,
                w_height, null, 0, w_width);
        int[] w_pixels = waterMark.getRGB(0, 0, w_width, w_height, null, 0,
                w_width);
        ...
        try {
                ...
                try {
                    for (int j = 0; j < w_width; j++) {
                        ...
                        if (waterPicType.equalsIgnoreCase("png")) {
                            if (!ignoreWhite || !((w_clip == 16777215
                                    && red == 255 && green == 255
                                    && blue == 255)
                                    || (red == 0 && green == 0 && blue == 0))) {
                                b_pixels[i * w_width + j] = (red * DarkPercent
                                        + b_red * (100 - DarkPercent)) / 100
                                        * 256 * 256;
                                b_pixels[i * w_width
                                        + j] += (green * DarkPercent
                                                + b_green * (100 - DarkPercent))
                                                / 100 * 256;
                                b_pixels[i * w_width + j] += (blue * DarkPercent
                                        + b_blue * (100 - DarkPercent)) / 100;
                            }
            ...
            basePicture.setRGB(position_x, position_y, w_width, w_height,
                    b_pixels, 0, w_width);
            return basePicture;
        } catch (IndexOutOfBoundsException e) {
            // 忽略列溢出的点
     ...
     * @return 打好水印的图片
     */
    public static BufferedImage markDarkPicture(BufferedImage basePicture,
            BufferedImage waterMark, int position_x, int position_y,
            int DarkPercent, boolean ignoreWhite) {
        ...
        try {
            for (int i = 0; i < w_height; i++) {
                try {
                    ...
                    for (int j = 0; j < w_width; j++) {
                        if ((i + j + position_x + position_y) % 2 != 0
                                && j != 0) {
                            /*
                            ...
                            if ((b_green - l_green + green) >= 0) {
                                b_pixels[i * w_width + j] += (b_green - l_green
                                        + green) > 255 ? 255 * 256
     ...
     * @return
     */
    public static BufferedImage gaussBlear(BufferedImage basePicture,
            int position_x, int position_y, int size_x, int size_y,
            int readSquare, int readCentralization) {
        int b_width = size_x;// basePicture.getWidth();
        int b_height = size_y;// basePicture.getHeight();
        int[] b_pixels = basePicture.getRGB(position_x, position_y, size_x,
                size_y, null, 0, size_x);
        if (readSquare < 0) {
            readSquare = 1;
        }
            ...
            try {
                for (int j = 0; j < b_width; j++) {
                        ...
                        for (int k = 1; k <= readSquare; k++) {
                            ...
                            for (int l = -k; l < k; l++) {
                                ...
                                if (l * l + k * k <= readSquare * readSquare) {
                                    ...
                                    int readTimes = readSquare - (int) Math
                                            .sqrt((double) (l * l + k * k));
                                    for (int m = 0; m < readTimes; m++) {
                                        readPixels.add((int) getPixel(b_pixels,
                                                b_width, j + l, i - k));
                                        readPixels.add((int) getPixel(b_pixels,
                                                b_width, j - l, i + k));
                                        readPixels.add((int) getPixel(b_pixels,
                    ...
                    for (Integer k : readPixels) {
                        read_red += cm.getRed((int) k) > 255 ? 255
                                : (cm.getRed((int) k) < 0 ? 0
                                        : cm.getRed((int) k));
                        read_green += cm.getGreen((int) k) > 255 ? 255
                                : (cm.getGreen((int) k) < 0 ? 0
                                        : cm.getGreen((int) k));
                        read_blue += cm.getBlue((int) k) > 255 ? 255
                                : (cm.getBlue((int) k) < 0 ? 0
                                        : cm.getBlue((int) k));
                    }
                    ...
                    // 按照参考强度算值
                    b_red = (b_red * readCentralization
                            + read_red * (100 - readCentralization)) / 100;
                    if (b_red > 255) {
                        b_red = 255;
                    }
                    if (b_red < 0) {
                    ...
                    b_green = (b_green * readCentralization
                            + read_green * (100 - readCentralization)) / 100;
                    if (b_green > 255) {
                        b_green = 255;
                    }
                    if (b_green < 0) {
                    ...
                    b_blue = (b_blue * readCentralization
                            + read_blue * (100 - readCentralization)) / 100;
                    if (b_blue > 255) {
                        b_blue = 255;
                    }
                    if (b_blue < 0) {
        ...
        basePicture.setRGB(position_x, position_y, size_x, size_y, b_pixels, 0,
                size_x);
        return basePicture;
     ...
     * @return
     */
    public static BufferedImage modifySight(BufferedImage basePicture,
            int position_x, int position_y, int size_x, int size_y, int range) {
        int b_width = basePicture.getWidth();
        int b_height = basePicture.getHeight();
        int[] b_pixels = basePicture.getRGB(position_x, position_y, size_x,
                size_y, null, 0, size_x);
        ...
        try {
            ...
            basePicture.setRGB(position_x, position_y, size_x, size_y, b_pixels,
                    0, size_x);
            return basePicture;
        } catch (IndexOutOfBoundsException e) {
            // 忽略列溢出的点
     ...
     * @return
     */
    public static BufferedImage modifyComparison(BufferedImage basePicture,
            int position_x, int position_y, int size_x, int size_y, int range) {
        int b_width = basePicture.getWidth();
        int b_height = basePicture.getHeight();
        int[] b_pixels = basePicture.getRGB(position_x, position_y, size_x,
            ...
            for (int i = 0; i < size_y; i++) {
                try {
                    for (int j = 0; j < size_x; j++) {
                        ...
                        int new_red = (red + 256 * range / 200) > 255 ? 255
                                : ((red + 256 * range / 200) < 127 ? 127
                                        : (red + 256 * range / 200));
                        if (red < 128) {
                            new_red = (red - 256 * range / 200) < 0 ? 0
                                    : ((red - 256 * range / 200) > 128 ? 128
                                            : (red - 256 * range / 200));
                        }
                        int new_green = (green + 256 * range / 200) > 255 ? 255
                                : ((green + 256 * range / 200) < 127 ? 127
                                        : (green + 256 * range / 200));
                        if (green < 128) {
                            new_green = (green - 256 * range / 200) < 0 ? 0
                                    : ((green - 256 * range / 200) > 128 ? 128
                                            : (green - 256 * range / 200));
                        }
                        int new_blue = (blue + 256 * range / 200) > 255 ? 255
                                : ((blue + 256 * range / 200) < 127 ? 127
                                        : (blue + 256 * range / 200));
                        if (blue < 128) {
                            new_blue = (blue - 256 * range / 200) < 0 ? 0
                                    : ((blue - 256 * range / 200) > 128 ? 128
                                            : (blue - 256 * range / 200));
                        }
     ...
     * @return
     */
    public static BufferedImage modifySaturation(BufferedImage basePicture,
            int position_x, int position_y, int size_x, int size_y, int range) {
        int b_width = basePicture.getWidth();
        int b_height = basePicture.getHeight();
        int[] b_pixels = basePicture.getRGB(position_x, position_y, size_x,
                    ...
                    for (int j = 0; j < size_x; j++) {
                                    ...
                                    : ((red + 256 * range / 200) < (green > blue
                                            ? green : blue)
                                                    ...
                                                    : (red + 256 * range
                                                            / 200));
                            if (green < blue) {
                                new_green = (green - 256 * range / 200) > blue
                                        ? blue
                                        : ((green - 256 * range / 200) < 0 ? 0
                                                : (green - 256 * range / 200));
                            } else {
                                new_blue = (blue - 256 * range / 200) > green
                                        ? green
                                        : ((blue - 256 * range / 200) < 0 ? 0
                                                : (blue - 256 * range / 200));
                            }
                        ...
                        if (amITheBiggest(green, red, blue)) {
                            new_green = (green + 256 * range / 200) > 255 ? 255
                                    : ((green + 256 * range / 200) < (red > blue
                                            ? red : blue)
                                                    ? (red > blue ? red : blue)
                                                    : (green + 256 * range
                                                            / 200));
                            if (red < blue) {
                                new_red = (red - 256 * range / 200) > blue
                                        ? blue
                                        : ((red - 256 * range / 200) < 0 ? 0
                                                : (red - 256 * range / 200));
                            } else {
                                new_blue = (blue - 256 * range / 200) > red
                                        ? red
                                        : ((blue - 256 * range / 200) < 0 ? 0
                                                : (blue - 256 * range / 200));
                            }
                                    ...
                                    : ((blue + 256 * range / 200) < (red > green
                                            ? red : green)
                                                    ...
                                                    : (blue + 256 * range
                                                            / 200));
                            if (red < green) {
                                new_red = (red - 256 * range / 200) > green
                                        ? green
                                        : ((red - 256 * range / 200) < 0 ? 0
                                                : (red - 256 * range / 200));
                            } else {
                                new_green = (green - 256 * range / 200) > red
                                        ? red
                                        : ((green - 256 * range / 200) < 0 ? 0
                                                : (green - 256 * range / 200));
                            }
     ...
     * @return
     */
    public static BufferedImage alterSize(BufferedImage source, int new_w,
            int new_h) {
        BufferedImage new_pic = new BufferedImage(new_w, new_h,
                BufferedImage.TYPE_INT_RGB);
                ...
                try {
                    for (int j = 0; j < new_w; j++) {
                        ...
                        int new_red = 0, new_green = 0, new_blue = 0,
                                pixelCount = 0;
                        for (int k = source_w * j / new_w; k < source_w
                                * (j + 1) / new_w; k++) {
                            for (int l = source_h * i / new_h; l < source_h
                                    * (i + 1) / new_h; l++) {
    ...
    public static Color getImageMainColor(BufferedImage source) {
        ...
        int k = 0;
        // 构建临时的temp的二维RGB图片数组
        int[] tempSourcePicPixelds = source.getRGB(0, 0, min_source_w,
        ...
        try {
            do {
                ...
                for (int i = 0; i < min_source_h; i++) {
                    try {
                        for (int j = 0; j < min_source_w; j++) {
                            Color clr = getColorType(getPixel(
                                    tempSourcePicPixelds, min_source_w, j, i));
                            if (clr == Color.red) {
                                colorlist[0]++;
                ...
                // 更改临时RGB里的数据
                tempSourcePicPixelds = source.getRGB(start_source_w_index,
                        start_source_h_index, min_source_w, min_source_h, null,
                        0, min_source_w);
            } while (k <= n - 1);
            /*
    ...
    public static BufferedImage reverseImageColor(BufferedImage source) {
        int source_w = source.getWidth();
        int source_h = source.getHeight();
        int[] sourcePicPixelds = source.getRGB(0, 0, source_w, source_h, null,
                0, source_w);
        try {
     ...
     * @return
     */
    public static BufferedImage reverseImage(BufferedImage source, boolean x,
            boolean y) {
        int source_w = source.getWidth();
        int source_h = source.getHeight();
        int[] sourcePicPixelds = source.getRGB(0, 0, source_w, source_h, null,
                    ...
                    try {
                        for (int j = 0; j < source_w / 2; j++) {
                            ...
                            int pixel = getPixel(sourcePicPixelds, source_w, j,
                                    i);
                            sourcePicPixelds[i * source_w
                                    + j] = sourcePicPixelds[i * source_w
                                            + source_w - j];
                            sourcePicPixelds[i * source_w + source_w
                                    - j] = pixel;
                        }
                    } catch (IndexOutOfBoundsException e) {
                        // 忽略行溢出的点
                    ...
                    try {
                        for (int j = 0; j < source_w; j++) {
                            ...
                            int pixel = getPixel(sourcePicPixelds, source_w, j,
                                    i);
                            sourcePicPixelds[i * source_w
                                    + j] = sourcePicPixelds[(source_h - i)
                                            * source_w + j];
                            sourcePicPixelds[(source_h - i) * source_w
                                    + j] = pixel;
                        }
                    } catch (IndexOutOfBoundsException e) {
                        // 忽略行溢出的点
     ...
     * @return
     */
    public static BufferedImage carveImage(BufferedImage source, int readpixel,
            int flagarea, boolean isNegative) {
        int source_w = source.getWidth();
        int source_h = source.getHeight();
        // 参考垂直点数
                ...
                try {
                    for (int j = 0; j < source_w; j++) {
                        ...
                        for (int k = 0; k < readpixels; k++) {
                            int readpixelup = getPixel(sourcePicPixelds,
                                    source_w, j, i - k);
                            int readpixeldown = getPixel(sourcePicPixelds,
                                    source_w, j, i + k);
                        ...
                        int readUp = readUp_red * 256 * 256 + readUp_green * 256
                                + readUp_blue;
                        int readDown = readDown_red * 256 * 256
                                + readDown_green * 256 + readDown_blue;

                        double distance = getLinearDistance(readUp, readDown);
                        int light = readUp_red + readUp_green + readUp_blue
                                - (readDown_red + readDown_green
                                        + readDown_blue);
                        if (light > flagArea) {
                            if (!isNegative) {
        ...
        source = new BufferedImage(source_w, source_h,
                BufferedImage.TYPE_INT_RGB);
        source.setRGB(0, 0, source_w, source_h, new_sourcePicPixelds, 0,
                source_w);
        return source;
    }
     ...
     * @return
     */
    public static BufferedImage cartoonImage(BufferedImage source,
            int readpixel, int flagarea, int borderPixel) {
        int source_w = source.getWidth();
        int source_h = source.getHeight();
        // 参考垂直点数
        int readpixels = readpixel;
            ...
            for (int i = 0; i < source_h; i++) {
                try {
                    for (int j = 0; j < source_w; j++) {
                        ...
                        int readLeft = readLeft_red * 256 * 256
                                + readLeft_green * 256 + readLeft_blue;
                        int readRight = readRight_red * 256 * 256
                                + readRight_green * 256 + readRight_blue;

                        distance = getLinearDistance(readLeft, readRight);
                        int light2 = readLeft_red + readLeft_green
                                + readLeft_blue - (readRight_red
                                        + readRight_green + readRight_blue);
                        // 判断边缘
                        if ((light > flagArea || light < -flagArea)
                                || (light2 > flagArea || light2 < -flagArea)) {
                            for (int l = 0; l < borderPixel; l++) {
                                if (light > flagArea || light < -flagArea) {
                                    new_sourcePicPixelds[(i - l) * source_w
                                            + j] = 0 * 256 * 256;
                                    new_sourcePicPixelds[(i - l) * source_w
                                            + j] += 0 * 256;
                                    new_sourcePicPixelds[(i - l) * source_w
                                ...
                                if (light2 > flagArea || light2 < -flagArea) {
                                    new_sourcePicPixelds[i * source_w + j
                                            + l] = 0 * 256 * 256;
                                    new_sourcePicPixelds[i * source_w + j
                                            + l] += 0 * 256;
                                    new_sourcePicPixelds[i * source_w + j
                            ...
                            new_sourcePicPixelds[i * source_w
                                    + j] = color.colorValue;
                        }
                    }
                } catch (IndexOutOfBoundsException e) {
     ...
     * @throws Exception
     */
    public static void saveJPG(BufferedImage source, String destPath)
            throws Exception {
        if (source != null) {
            ImgFileUtils.makeDirectory(destPath);
     ...
     * @throws Exception
     */
    public static void savePNG(BufferedImage source, String destPath)
            throws Exception {
        if (source != null) {
            ImgFileUtils.makeDirectory(destPath);
            FileOutputStream fout = new FileOutputStream(destPath);
    ...
    public static Color getColorType(int color) {
        ...
        double minValue = Math.min(toRed, Math.min(toOrange, Math.min(toYellow,
                Math.min(toGreen, Math.min(toCyan, Math.min(toBlue,
                        Math.min(toPurple, Math.min(toWhite, toBlack))))))));
     ...
     * @return
     */
    public static BufferedImage cutImage(BufferedImage image, int index_x,
            int index_y, int width, int height) throws Exception {
     ...
     * @return
     */
    public static BufferedImage alterSize(BufferedImage image,
            int zoomInPercent) {
        return alterSize(image, image.getWidth() * zoomInPercent / 100,
                image.getHeight() * zoomInPercent / 100);
    }
     ...
     * @return
     */
    public static BufferedImage splitCell(BufferedImage image, int x_percent,
            int y_percent, int percent) {
        ...
        image.setRGB(s_width * x_percent / 100, s_height * y_percent / 100,
                s_width * percent / 100,
                s_height * percent / 100, child.getRGB(0, 0, child.getWidth(),
                        child.getHeight(), null, 0, child.getWidth()),
                0, child.getWidth());
        return image;
    }
    ...
    public static BufferedImage rotateImage(BufferedImage bufferedimage,
        ...
        (graphics2d = (img = new BufferedImage(dest_w, dest_h, type))
    ...
    public static BufferedImage turnImage(BufferedImage source, double angle) {
        ...
        int source_w = source.getWidth();
        int source_h = source.getHeight();
        double rotateRound = Math
                .sqrt((double) (source_w * source_w + source_h * source_h));
        // 旋转矩阵
            ...
            for (int i = 0; i < source_h; i++) {
                try {
                    for (int j = 0; j < source_w; j++) {
                        ...
                        location[1][0] = i;
                        location[2][0] = 1;
                        newLocation[0][0] = rotateMatrix[0][0] * location[0][0]
                                + rotateMatrix[0][1] * location[1][0]
                                + rotateMatrix[0][2] * location[2][0];
                        newLocation[1][0] = rotateMatrix[1][0] * location[0][0]
                                + rotateMatrix[1][1] * location[1][0]
                                + rotateMatrix[1][2] * location[2][0];
                        newLocation[2][0] = rotateMatrix[2][0] * location[0][0]
                                + rotateMatrix[2][1] * location[1][0]
                                + rotateMatrix[2][2] * location[2][0];
                        int integerX = newLocation[0][0] > 0
                                ? (int) newLocation[0][0]
                                : ((int) newLocation[0][0] - 1);
                        int integerY = newLocation[1][0] > 0
                                ? (int) newLocation[1][0]
                                : ((int) newLocation[1][0] - 1);
                        double[][] colorPercent = new double[2][2];
                        colorPercent[0][0] = 1 - Math.sqrt((newLocation[0][0]
                                - integerX) * (newLocation[0][0] - integerX)
                                + (newLocation[1][0] - integerY)
                                        * (newLocation[1][0] - integerY));
                        if (colorPercent[0][0] < 0) {
                            colorPercent[0][0] = 0;
                        }
                        colorPercent[0][1] = 1 - Math.sqrt((integerX + 1
                                - newLocation[0][0])
                                * (integerX + 1 - newLocation[0][0])
                                + (newLocation[1][0] - integerY)
                                        * (newLocation[1][0] - integerY));
                        if (colorPercent[0][1] < 0) {
                            colorPercent[0][1] = 0;
                        }
                        colorPercent[1][0] = 1 - Math.sqrt((newLocation[0][0]
                                - integerX) * (newLocation[0][0] - integerX)
                                + (integerY + 1 - newLocation[1][0])
                                        * (integerY + 1 - newLocation[1][0]));
                        if (colorPercent[1][0] < 0) {
                            colorPercent[1][0] = 0;
                        }
                        colorPercent[1][1] = 1 - Math.sqrt((integerX + 1
                                - newLocation[0][0])
                                * (integerX + 1 - newLocation[0][0])
                                + (integerY + 1 - newLocation[1][0])
                                        * (integerY + 1 - newLocation[1][0]));
                        ...
                        destPixel = destPicPixelds[(integerY + dest_h / 2)
                                * dest_w + integerX + dest_w / 2];
                        ...
                        destPicPixelds[(integerY + dest_h / 2) * dest_w
                                + integerX + dest_w / 2] = mixColor(destRed,
                                        destGreen, destBlue);

                        destPixel = destPicPixelds[(integerY + dest_h / 2)
                                * dest_w + integerX + 1 + dest_w / 2];
                        destRed = cm.getRed(destPixel);
                        destGreen = cm.getGreen(destPixel);
                        destBlue = cm.getBlue(destPixel);
                        ...
                        destPicPixelds[(integerY + dest_h / 2) * dest_w
                                + integerX + 1 + dest_w / 2] = mixColor(destRed,
                                        destGreen, destBlue);

                        destPixel = destPicPixelds[(integerY + 1 + dest_h / 2)
                                * dest_w + integerX + dest_w / 2];
                        destRed = cm.getRed(destPixel);
                        destGreen = cm.getGreen(destPixel);
                        destBlue = cm.getBlue(destPixel);
                        ...
                        destPicPixelds[(integerY + 1 + dest_h / 2) * dest_w
                                + integerX + dest_w / 2] = mixColor(destRed,
                                        destGreen, destBlue);
    ...
    private static BufferedImage trimBorder(BufferedImage source) {
        ...
        newImg = source.getSubimage(x_start, y_start, x_end - x_start,
                y_end - y_start);
        source = null;
        return newImg;
     ...
     *            旋转角度，90的倍数
     */
    public static void rotateImage(String srcFilename, String destPath,
            int degree, int siteId, SysConfigService sysConfigService)
            throws Exception {
        try {
            BufferedImage bi = getGraphics(srcFilename, siteId,
                    sysConfigService);
     ...
     * @throws Exception
     */
    public static void gmRotateImage(String srcFilename, String destPath,
     ...
     *            高度
     */
    public static void cropImage(String srcFilename, String destPath, int x,
            int y, int w, int h, int siteId, SysConfigService sysConfigService)
            throws Exception {
        try {
            BufferedImage bi = getGraphics(srcFilename, siteId,
                    sysConfigService);
     ...
     * @throws Exception
     */
    public static BufferedImage rotateImageIgnoreSize(BufferedImage source,
        ...
        try {
            ...
            System.gc();
            int new_size = source.getWidth() > source.getHeight()
     ...
     * @throws IOException
     */
    public static void waterMarkPic(String waterPic, String srcPic,
            String waterMarkerPic, String place, Boolean synFlag)
    ...
    public static int[] computeSize(BufferedImage alterdImage,
            int picRealSize) {
        int[] size = new int[2];
     ...
     * @return
     */
    public static BufferedImage computePosAndMark(int isWaterMark, int postion,
            String wMPath, BufferedImage waterPic, BufferedImage alterdImage) {
        ...
        try {
            if (isWaterMark == 0) {
                ...
                alterdImage = ImageAnalyseUtil.markPicture(alterdImage,
                        waterPic, posW, posH, 50, true, "png");
    ...
    public static BufferedImage resize(BufferedImage alterdImage, int pxH) {
        ...
        if (pxH > height && pxH > width) {
            ...
            if (height > width) {
                ...
                image = new BufferedImage((int) newWidth, pxH,
                        BufferedImage.TYPE_INT_BGR);
                Graphics graphics = image.createGraphics();
                graphics.drawImage(alterdImage, 0, 0, (int) newWidth, pxH,
                        null);
            }
            if (height < width) {
                double percent = width / pxH;
                double newHeight = height / percent;
                image = new BufferedImage(pxH, (int) newHeight,
                        BufferedImage.TYPE_INT_BGR);
                Graphics graphics = image.createGraphics();
                graphics.drawImage(alterdImage, 0, 0, pxH, (int) newHeight,
                        null);
            }
            if (height == width) {
