...
public class LoginController {
    ...
    public Object getSalt(HttpServletRequest request, String userName) {
        ...
        try {
            ...
            // 存入redis，salt五分钟失效
            redisClientTemplate.setex(CommonConstant.USERSALT + userName,
                    CommonConstant.FAILUREMILLSSALT, salt);
    ...
    public Object doLogin(HttpServletRequest request, String userName,
        ...
        try {
            ...
            if (lowerCase.equalsIgnoreCase(vilidate)
                ...
                if (salt == null) {
                    ...
                    if (user != null) {
                        String lockTime = sysConfigService.getDbSysConfig(
                                SysConfigConstant.UNLOCKTIME, user.getSiteId());
                        // 登录成功
                        ...
                        if (status == 3) {
                            ...
                            String pwd_md5 = Coder
                                    .reverse(Coder.decryptBASE64(pwd_db));// md5密码
                            // System.out.println("--------------------------");
                            ...
                            if (password.equals(pwd)) {
                                List<CpRole> role = basicMapper
                                        .selectUserRoleByUId(user.getId());// 查询用户角色
                                // 更新用户的最后一次登录时间
                                CpUser newUser = new CpUser();
                                newUser.setId(user.getId());
                                newUser.setLogincount(
                                ...
                                newUser.setRolenames(
                                        UserUtils.setUserRoleNames(role));
                                cpUserMapper
                                        .updateByPrimaryKeySelective(newUser);// 修改登录时间
                                ...
                                if (role != null && role.size() > 0) {
                                    ...
                                    right = basicMapper
                                            .selectAllRightByUserId(map);// 取第一个角色的权限
                                }
                                CpUser cpUser = cpUserMapper
                                ...
                                res.setData(dt);
                                res.setOther(
                                        String.format("用户名为【%s】", userName));
                                // 登录成功后，将在线用户存放在redis中
                                String redisId = CommonConstant.REDISID
                                        + user.getId();
                                // 踢掉另一个正在登陆的相同用户
                                if (redisClientTemplate.exists(redisId)) {
                                    String userToken = redisClientTemplate
                                            .get(redisId);
                                ...
                                redisClientTemplate.setex(redisId,
                                        CommonConstant.FAILUREMILLSTOKEN,
                                        userToken);// 存放用户令牌
                                redisClientTemplate.setex(userToken,
                                        CommonConstant.FAILUREMILLSTOKEN,
                                        GsonUtil.toJson(cpUser));// 令牌存放在线用户信息
                                redisClientTemplate.lrem(
                                        CommonConstant.ONLINUSERLIST, 0,
                                        redisId);// 移除已有list
                                // 将在线用户id，放在list中，为在线用户列表
                                redisClientTemplate.lpush(
                                        CommonConstant.ONLINUSERLIST, redisId);
                                // 设置到cookie
                                ...
                                if (user.getLoginNo() != null
                                    ...
                                    Integer loginNo = basicMapper
                                            .selectPwdByUserName(param)
                                            .getLoginNo();
                                    res.setMsg("密码错误 ,您还有" + (5 - loginNo)
                                            + "次机会");
                                }
                            }
                        } else {
                ...
                // 记录日志
                // edit by liu.jinfeng@20170903 修改获取IP地址方法
                // String ip = request.getHeader("X-Real-IP");
                String ip = UserUtils.getLocalIp(request);

                int siteId = (Integer) SessionUtils.getSiteId(request);
                StringBuffer params = new StringBuffer();
                params.append(JsonUtil.getString(userName)).append(";")
                        .append(JsonUtil.getString(password));
                CpLog log = new CpLog();
                log.setOpeContent("用户登录");
                log.setLogtypeId(logService
                        .getLogTypeIdByCode(CommonConstant.UserLogin));
                log.setOpeIp(ip);
    ...
    public Object getOnLineUsers(HttpServletRequest request, Integer rows,
            Integer page) {
        ResponseMessage res = new ResponseMessage();
        try {
            ...
            if (redisClientTemplate.exists(CommonConstant.ONLINUSERLIST)) {
                List<String> list = redisClientTemplate
                        .lrange(CommonConstant.ONLINUSERLIST, 0, -1);
                e = e > list.size() ? list.size() : e;
                for (int i = f; i < e; i++) {
                    redisId = list.get(i);
                    if (redisClientTemplate.exists(redisId)) {// 在线用户
                        userToken = redisClientTemplate.get(redisId);
                        userinfo = (CpUser) GsonUtil.fromJson(
                                redisClientTemplate.get(userToken),
                                CpUser.class);
                        if (userinfo != null) {
                            /*
                        ...
                        redisClientTemplate.lrem(CommonConstant.ONLINUSERLIST,
                                0, redisId);// 移除list
                    }
                    if (data.size() == rowsI) {
                        break;
        ...
        try {
            ...
            if (null != user) {
                ...
                if (redisClientTemplate.exists(redisId)) {
                    ...
                    redisClientTemplate.del(userToken);
                    // 从在线列表中删除用户名
                    redisClientTemplate.lrem(CommonConstant.ONLINUSERLIST, 0,
                            redisId);// 移除list
                } else {
                    // 从在线列表中删除用户名
                    redisClientTemplate.lrem(CommonConstant.ONLINUSERLIST, 0,
                ...
                res.setCode(CommonConstant.SUCCESSCODE);
                res.setMsg("退出登陆成功");
                res.setOther(String.format("userId=%s,userName=%s",
    ...
    public Object logoutUser(HttpServletRequest request, String userId) {
        ...
        try {
            ...
            if (redisClientTemplate.exists(redisId)) {
                ...
                if (userS != null) {
                    CpUser user = (CpUser) GsonUtil.fromJson(
                            redisClientTemplate.get(userToken), CpUser.class);
                    if (user != null) {
    ...
    public Object register(HttpServletRequest request, CpUser user,
            Integer roleId,String code) {
        ...
        try {
            ...
            if (roleId.equals("4")
                    && user.getFeeType() == CommonConstant.BYTE0) {// 摄影师-邮寄
                CommonValidation.checkParamBlank(user.getMailAddress(), "通信地址");
                CommonValidation.checkParamBlank(user.getMailUsername(),
                        "收稿费人姓名");
                CommonValidation.checkParamBlank(user.getMailIdCard(),
                        "收稿费人身份证号");
                CommonValidation.checkParamBlank(user.getMailZipCode(),
                        "邮寄邮政编码");
            }
            if (roleId.equals("4")
                    && user.getFeeType() == CommonConstant.BYTE1) {// 摄影师-电汇
                CommonValidation.checkParamBlank(user.getBankAccount(),
                        "邮政储蓄存折帐号或邮政储蓄卡号");
                CommonValidation.checkParamBlank(user.getBankUsername(),
                        "上述卡的户名");
                CommonValidation.checkParamBlank(user.getBankIdCard(),
                        "上述卡户名的身份证号");
            ...
            if (list != null && list.size() > 0) {
                ...
                result.setMsg(
                        String.format("用户名为【%s】已存在！", user.getUserName()));
                return result;
            }
...
//            log.info("PHONE"+user.getTelBind()+code);
            ...
            user.setPassword(
                    Coder.encryptBASE64(Coder.reverse(user.getPassword())));
            ...
            result.setOther(String.format("userId=%s;roleIds=%s", user.getId(),
                    roleId));
    ...
    @SkipLoginCheck
    @SkipAuthCheck
    public Object findUserByInfo(HttpServletRequest request, String userInfo,
            String vilidate, HttpServletResponse response) {
        ...
        try {
            ...
            if (scode == null) {
                ...
                if (lowerCase.equalsIgnoreCase(vilidate)
                    ...
                    if (user == null) {
                        // 匹配手机号
                        List<CpUser> userList = cpUserMapper
                                .findUserByBindTel(userInfo);

                        if (userList == null || userList.size() == 0) {
                            // 匹配邮箱
                            userList = cpUserMapper
                                    .findUserByBindEmail(userInfo);
                            if (userList == null || userList.size() == 0) {
                                ...
                                map.put("telBind", getRegexInfo(1,
                                        userList.get(0).getTelBind()));
                                map.put("emailBind", getRegexInfo(2,
                                        userList.get(0).getEmailBind()));
                                result.setData(map);
                                result.setCode(CommonConstant.SUCCESSCODE);
                                result.setMsg(CommonConstant.SUCCESSSTRING);
                            }
                        ...
                        map.put("emailBind",
                                getRegexInfo(2, user.getEmailBind()));
                        result.setData(map);
                        result.setCode(CommonConstant.SUCCESSCODE);
                        result.setMsg(CommonConstant.SUCCESSSTRING);
                    }
    ...
    public Object submitNewPassword(HttpServletRequest request, String mark,
        ...
        try {
            ...
            String markParm = redisClientTemplate.get("mark" + userName)
                    .toString().replace("\r\n", "");
            // 解密标志
            String DecryptMark = MD5Util2
                    .convertMD5(Coder.reverse(Coder.decryptBASE64(markParm)));
            // 验证通过
            if (mark.replace("\r\n", "").equals(markParm)
                    && DecryptMark.contains(userName)) {
                CpUser user = cpUserMapper.findUserByUserName(userName);
                String oldPassword = Coder
                        .encryptBASE64(Coder.reverse(newPassword));
                // 新密码与原密码是否一致
                if (!(oldPassword.replace("\r\n", ""))
                    ...
                    cpUser.setPassword(
                            Coder.encryptBASE64(Coder.reverse(newPassword)));
                    ...
                    try {
                        ...
                        List<String> Email = configService.findEmail(
                                SysConfigConstant.Email_adds,
                        ...
                        // 连接邮件服务器
                        try {
                            transport.connect("smtp.263.net", Email.get(0),
                                    Email.get(1));
                            // 发送邮件s
    ...
    public static void main(String[] args) {
        ...
        // 8)+users.get(0).getUserName();
        // String reverse =
        // Coder.encryptBASE64(Coder.reverse(MD5Util2.string2MD5(markStr)));
    ...
    @SkipLoginCheck
    @SkipAuthCheck
    public Object findPassword(HttpServletRequest request, String userName,
            Integer status, String phone, String vilidate, String email,
        ...
        try {
            if (status == 1) {// status=1 是登录名找回
                ...
                String newPassword = UUID.randomUUID().toString().substring(0,
                        8);
                String reverse = Coder.encryptBASE64(
                        Coder.reverse(MD5Util2.string2MD5(newPassword)));
                cpUser.setPassword(reverse);
                cpUserMapper.updateByPrimaryKeySelective(cpUser);
                mailController.findPasswordByEmail(request, userName,
                        newPassword, cpUser.getEmailBind(), cpUser.getId());
            }
            if (status == 2) {// status=2是手机找回 需要输入登录名
                String attribute = (String) request.getSession()
                        .getAttribute("vilidate");
                CommonValidation.checkParamBlank(vilidate, "验证码");
                ...
                if (attribute.equalsIgnoreCase(vilidate)) {
                    ...
                    cpUser.setPassword(reverse);
                    cpUserMapper.updateByPrimaryKeySelective(cpUser);
                    phoneMSGController.findPasswordByPhone(request,
                            cpUser.getId(), newPassword, cpUser.getTelBind());
            ...
            if (status == 4) {// status=4密码提示 需要输入登录名才能得到提问
                ...
                for (CpUser cpUser2 : list) {
                    if (cpUser2.getAnswer().equals(answer)) {
                        String newPassword = UUID.randomUUID().toString()
                                .substring(0, 8);
                        String reverse = Coder.encryptBASE64(Coder
                                .reverse(MD5Util2.string2MD5(newPassword)));
                        cpUser2.setPassword(reverse);
                        cpUserMapper.updateByPrimaryKeySelective(cpUser2);
                        phoneMSGController.findPasswordByPhone(request,
                                cpUser2.getId(), newPassword,
                                cpUser2.getTelBind());
                    } else {
                        result.setCode(CommonConstant.PARAMERROR);
    ...
    @SkipLoginCheck
    @SkipAuthCheck
    public Object getPasswordPrompt(HttpServletRequest request,
            String userName) {
        ResponseMessage result = new ResponseMessage();
        try {
            CommonValidation.checkParamBlank(userName, "登录名");
    ...
    public void logLogin(HttpServletRequest request, String userName,
            ResponseMessage res, String password) {
        ...
        log.setOpeContent("用户登录");
        log.setLogtypeId(
                logService.getLogTypeIdByCode(CommonConstant.UserLogin));
        log.setOpeIp(ip);
        log.setOpeTime(new Date());
        log.setOpeType(CommonConstant.BYTE2);
        log.setOpeParam(params.toString());
    ...
    @SkipLoginCheck
    @SkipAuthCheck
    public Object xiaohehe(HttpServletRequest request, String userName,
            String password) {
    ...
    public Object testRedis(HttpServletRequest request,
        ...
        try {
             ...
             * redisClientTemplate.set("redisId_2", uuid); String
             * json=redisClientTemplate.get(uuid); redisClientTemplate.set(uuid,
             * json);
             */
