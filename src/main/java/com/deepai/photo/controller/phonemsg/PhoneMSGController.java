...
public class PhoneMSGController {
	...
	@SkipLoginCheck
	public Object sendMessageByUserName(HttpServletRequest request,String userName, String password,Integer roleId){
		...
		try {
			...
			if(pwd_md5.equals(password)){
				...
				List<String> phoneMSG = configService.findEmail(SysConfigConstant.Phone_username,
						SysConfigConstant.Phone_password);
				//h3235055,geagag11515gag2t2gaegaga 这是原来的用户名密码
				try {
					
					String msgResult = "";
					msgResult = phoneMSGService.sendSMS(cpUser.getTelBind(), content,phoneMSG.get(0) , phoneMSG.get(1));
	...
	public Object send(HttpServletRequest request, String uids, String content, String teamIds, int status)
		...
		try {
			...
			//h3235055,geagag11515gag2t2gaegaga 这是原来的用户名密码
			try {
				List<String> phoneN = userRoleRightService.findPhoneByUid(list2);
				...
				for (int i = 0; i < num-1; i++) {
					phones = StringUtils.join(phoneN.subList(0+i*5000, 49999+i*50000), ",");
					msgResult = phoneMSGService.sendSMS(phones, content,phoneMSG.get(0) , phoneMSG.get(1));
					log.info("短信发送结果----------> "+msgResult );
					msgResult=msgResult.replace('\n', ',');
					result2 = msgResult.split(",")[1];
					if ("0".equalsIgnoreCase(result2)) { // 如果发送成功就保存到数据库
				...
				phones = StringUtils.join(phoneN.subList(0+(num-1)*50000, phoneN.size()), ",");
				msgResult = phoneMSGService.sendSMS(phones, content,phoneMSG.get(0) , phoneMSG.get(1));
				log.info("短信发送结果----------> "+msgResult );
				msgResult=msgResult.replace('\n', ',');
				...
				/*for (int i = 0; i <num; i++) {
					msgResult = phoneMSGService.sendSMS(phoneN.get(i), content,phoneMSG.get(0) , phoneMSG.get(1));
					msgResult=msgResult.replace('\n', ',');
				...
				if ("0".equalsIgnoreCase(result2)) { // 如果发送成功就保存到数据库
					...
					save(uid, content, teamId, request, 1,phoneN.get(i)); // 发送失败保存
					result.setCode(CommonConstant.EXCEPTIONCODE);
					result.setMsg("短信发送失败");
					break;
				}
	...
	public Object detail(HttpServletRequest request, HttpServletResponse response, int id) {
		...
		try {
			...
			String datetime = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(cpPhoneMsg.getCreateTime());
	...
	public Object search(HttpServletRequest request, HttpServletResponse response, String search,
			String sender,Integer status,String timeFrom,String timeTo) {
		ResponseMessage result = new ResponseMessage();
		try {
	    PageHelper.startPage(request);
	    List<CpPhoneMsg> list = phoneMSGService.search(search,sender,status,timeFrom,timeTo);
	    PageHelper.addPages(result, list);
		for (int i = 0; i < list.size(); i++) {
			 String datetime = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(list.get(i).getCreateTime());
				list.get(i).setDatetime(datetime);
	...
	public void save(int[] uid, String content, int[] teamId, HttpServletRequest request, int Status,String phoneNum) {
		String roseid = "";
		String userid = "";
		if (teamId!=null&&teamId.length > 0) {
			for (int i : teamId) {
	...
	@SkipLoginCheck
	@SkipAuthCheck
	public Object findPasswordByPhone(HttpServletRequest request,  int userId, String newPassword, String userPhone)
			throws HttpException, IOException {
		ResponseMessage result = new ResponseMessage();
...
//			SendChit sendChit = new SendChit();
			String  sendResult=phoneMSGService.sendSMS(userPhone, content,phoneMSG.get(0),phoneMSG.get(1));
...
/*			
			...
			for (int i = 0; i < phone.size(); i++) {
				NameValuePair[] data = { new NameValuePair("Uid", phoneMSG.get(0)), // 注册的用户名
				...
				result2 = new String(post.getResponseBodyAsString().getBytes("gbk"));
			}
*/
				CpPhoneMsg cpPhoneMsg = new CpPhoneMsg();
	...
	@SkipLoginCheck
	public Object confirmPhoneVilidate(HttpServletRequest request,String userName
			,String vilidate){
		ResponseMessage result = new ResponseMessage();
		...
		try {
			if(redisClientTemplate.ttl("PHONE"+userName+vilidate)==-2){
				...
				String phoneVilidate = redisClientTemplate.get("PHONE"+userName+vilidate);
...
//			}
			...
			redisClientTemplate.set("PHONE"+userName+vilidate, vilidate+"");
			redisClientTemplate.expire("PHONE"+userName+vilidate, 60*2);
			
//			//清除过期key值
//			request.getSession().setAttribute("phoneVilidate", vilidate);
//			redisClientTemplate.expire("EMAIL"+userName+request.getSession().getAttribute("phoneVilidate"), -2);
//			request.getSession().setAttribute("phoneVilidate", vilidate);
			String content="【中新社】 您正在执行中新社密码找回，验证码是: "+vilidate+"。请按页面提示提交验证码，切记请勿将验证码泄露给他人。";

			String  sendResult=phoneMSGService.sendSMS(user.getTelBind(), content,phoneMSG.get(0),phoneMSG.get(1));
			sendResult=sendResult.replace('\n', ',');
...
//			SendChit sendChit = new SendChit();
			String  sendResult=phoneMSGService.sendSMS(phoneNum, content,phoneMSG.get(0),phoneMSG.get(1));
			sendResult=sendResult.replace('\n', ',');
