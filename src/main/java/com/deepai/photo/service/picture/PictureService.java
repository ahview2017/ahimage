...
public class PictureService {
    ...
    public List<CpPicture> uploadMorePic(MultipartFile[] picFiles, int siteId)
        ...
        for (int i = 0; i < picFiles.length; i++) {
            ...
            } else if (name.toLowerCase().lastIndexOf("tif") >= name.length()
                    - 4) {
                filename = getFileName("tif");
            } else if (name.toLowerCase().lastIndexOf("png") >= name.length()
                    - 4) {
             ...
             * map.put("fileTime",pic.getFilmTime()); map.put("samllPath",
             * CommonConstant.SMALLHTTPPath+ImgFileUtils.getSamllPathByName(
             * filename)); map.put("height",pic.getPictureHeight());
    ...
    public String getPicFileName(String oriName) throws Exception {
        ...
        if (oriName.toLowerCase().lastIndexOf("jpg") >= oriName.length() - 4) {
            filename = getFileName("jpg");
        } else if (oriName.toLowerCase().lastIndexOf("tif") >= oriName.length()
                - 4) {
            filename = getFileName("tif");
        } else if (oriName.toLowerCase().lastIndexOf("png") >= oriName.length()
                - 4) {
            filename = getFileName("png");
        } else {
            filename = getFileName("jpg");
     ...
     * @throws Exception
     */
    public String uploadPushPic(String fileName, CpPicAllpath cpPicAllpath,
        ...
        if (cpPicAllpath.getAllPath().contains("classification")) {
            oriPath = sysConfigService.getDbSysConfig(
                    SysConfigConstant.DEFAULT_CLASSIFICATION_PATH, siteId);
        } else if (cpPicAllpath.getAllPath().contains("watermarkedmedium")) {
            oriPath = sysConfigService.getDbSysConfig(
                    SysConfigConstant.WATERMARKEDMEDIUM_PIC_PATH, siteId);
        } else if (cpPicAllpath.getAllPath().contains("big")) {
            oriPath = sysConfigService
                    .getDbSysConfig(SysConfigConstant.BIG_PIC_PATH, siteId);
        } else if (cpPicAllpath.getAllPath().contains("medium")) {
            oriPath = sysConfigService
                    .getDbSysConfig(SysConfigConstant.MEDIUM_PIC_PATH, siteId);
        } else if (cpPicAllpath.getAllPath().contains("small")) {
            oriPath = sysConfigService
                    .getDbSysConfig(SysConfigConstant.SMALL_PIC_PATH, siteId);
        }
        ...
        String name = FileUpload.fileUpNameByInputStream(fis, oriPath,
                fileName);
    ...
    public CpPicture uploadOnePic(MultipartFile picFile, String filename,
        ...
        // add by liu.jinfeng@20170903
         identifyCmd.setSearchPath("/usr/local/Cellar/graphicsmagick/1.3.26_1/bin");
//         identifyCmd.setSearchPath("/usr/local/graphicsmagick/bin");
        ...
        if (out.size() > 0) {
            ...
            // 上传图片宽高限制
            String minWidth = sysConfigService
                    .getDbSysConfig(SysConfigConstant.PICTUREMINWIDTH, siteId);// 最小宽
            if (width < Integer.valueOf(minWidth)) {
                throw new InvalidHttpArgumentException(
                        CommonConstant.FAILURECODE,
                        String.format("图片宽不能小于【%s】像素", minWidth));
            }
            String minHeight = sysConfigService
                    .getDbSysConfig(SysConfigConstant.PICTUREMINHEIGHT, siteId);// 最小高
            if (height < Integer.valueOf(minHeight)) {
                throw new InvalidHttpArgumentException(
                        CommonConstant.FAILURECODE,
                        String.format("图片高不能小于【%s】像素", minHeight));
            }
            if (arr.length > 2) {
                time = arr[2];
            }
        ...
        // 处理小图，返回小图原路径
        String smallAllPath = opeTmpSmall(oriAllPath, width, height, false,
                siteId);
        if (width > height) {
        ...
        if (filename.toLowerCase().lastIndexOf("tif") >= filename.length()
                - 4) {
        ...
        if (saveFile.length() < minL) {
            throw new InvalidHttpArgumentException(CommonConstant.FAILURECODE,
                    String.format("图片大小不能小于【%s】KB", minM));
        }
        String maxM = sysConfigService
                .getDbSysConfig(SysConfigConstant.PICTUREMAXLENGTH, siteId);
        Integer maxMI = Integer.valueOf(maxM);// 图片大小 最大限制 单位KB
        Long maxL = maxMI * 1024L;// 单位字节
        if (saveFile.length() > maxL) {
            throw new InvalidHttpArgumentException(CommonConstant.FAILURECODE,
                    String.format("图片大小不能超过【%s】KB", maxM));
        }
        ...
        /*
         * if(groupId!=null){ pic.setGroupId(groupId); }
         */
        ...
        // 生成中图
        int mediumSize = Integer.valueOf(sysConfigService
                .getDbSysConfig(SysConfigConstant.MEDIUM_PIC_SIZE, siteId));
        String mediumPath = sysConfigService
                .getDbSysConfig(SysConfigConstant.MEDIUM_PIC_PATH, siteId);
        ...
        if (mediumSize <= picSize) {
            ImageAnalyseUtil.gmAlterImg(mediumSize, oriAllPath,
                    initFullPathByOrder(mediumPath, filename), width, height,
                    false);
        } else {
     ...
     * @throws Exception
     */
    private String opeTmpSmall(String Path, int width, int height, boolean syn,
            int siteId) throws Exception {
        int picSize = width > height ? width : height;
        ...
        int smallPicSize = Integer.parseInt(sysConfigService
                .getDbSysConfig(SysConfigConstant.SMALL_PIC_SIZE, siteId));// 系统配置小图大小
        String smallPicPath = sysConfigService
                .getDbSysConfig(SysConfigConstant.SMALL_PIC_PATH, siteId);// 系统配置小图路径
        String allPath = initFullPathByOrder(smallPicPath, fileName);
        if (smallPicSize <= picSize) {
            ImageAnalyseUtil.gmAlterImg(smallPicSize, Path, allPath, width,
                    height, syn);
        } else {
    ...
    public String initFullPathByOrder(String root, String fileName) {
        ...
        if (System.getProperty("os.name").toUpperCase()
                .indexOf("WINDOWS") != -1) {
            if (tempp.lastIndexOf(CommonConstant.oneSprit) != tempp.length()
                    - 1) {
                tempp += CommonConstant.oneSprit;
            }
            if (tempp.lastIndexOf(CommonConstant.doubleSprit) == tempp.length()
                    - 1) {
    ...
    public String initFullPathNoFile(String root, String fileName) {
        ...
        if (System.getProperty("os.name").toUpperCase()
            ...
            tempp = tempp + fileName.substring(0, 4)
                    + CommonConstant.doubleSprit + fileName.substring(0, 8)
                    + CommonConstant.doubleSprit;
        } else {
            if (tempp.lastIndexOf(CommonConstant.oneSprit) != tempp.length()
                    - 1) {
                tempp += CommonConstant.oneSprit;
    ...
    private String getFileName(String type) throws Exception {
        ...
        String picNum_ = String.format("%s%06d", DateUtils.getNowShortDate(),
                picNum);
     ...
     * @throws Exception
     */
    public CpPicture loadSinglePicInfo(CpPicture cpPicture, boolean isIpTc,
            int siteId) throws Exception {
        ...
        try {
            ...
            CpPicture oldPic = cpPictureMapper
                    .selectByPrimaryKey(cpPicture.getId());
            boolean synFlag = false;// 默认不是异步处理，否则前端会发生不返回数据的情况
            long st = System.currentTimeMillis();
            cpPicture.setIsSourcePicture(CommonConstant.BYTE1);
            // 生成大、中、水印图
            opeMediumandSmall(oriAllPath, oldPic.getPictureWidth(),
                    oldPic.getPictureHeight(), synFlag, siteId,
                    cpPicture.getId());
    ...
    private CpPicture loadInfoByEXIF(CpPicture ap, File temp) {
        try {
            Directory EXIFInfo = ImageAnalyseUtil
                    .extratEXIFFromFile(temp.getAbsolutePath());
            // 型号
            ap.setExModel(EXIFInfo.getString(ExifDirectory.TAG_MODEL));
            // 快门
            ap.setExExposureTime(
                    EXIFInfo.getString(ExifDirectory.TAG_EXPOSURE_TIME));
            try {
                ap.setExDatetime(
                        new SimpleDateFormat("yyyy:MM:dd HH:mm:ss", Locale.US)
                                .parse(EXIFInfo.getString(
    ...
    private CpPicture loadInfoByIPTC(CpPicture ap, File temp) throws Exception {
        try {
            Directory IPTCInfo = ImageAnalyseUtil
                    .extratIPTCFromFile(temp.getAbsolutePath());
            Boolean isnull = true;
            System.out.println(IPTCInfo.getString(IptcDirectory.TAG_CAPTION));
            isnull = shiftCase(isnull,
                    IPTCInfo.getString(IptcDirectory.TAG_CAPTION));
            isnull = shiftCase(isnull,
                    IPTCInfo.getString(IptcDirectory.TAG_KEYWORDS));
            isnull = shiftCase(isnull,
                    IPTCInfo.getString(IptcDirectory.TAG_CITY));
            isnull = shiftCase(isnull,
                    IPTCInfo.getString(IptcDirectory.TAG_HEADLINE));
            isnull = shiftCase(isnull,
                    IPTCInfo.getString(IptcDirectory.TAG_CATEGORY));
            isnull = shiftCase(isnull,
            ...
            if (fileName.toLowerCase().indexOf("tif") >= fileName.length()
                    - 4) {
                fileName = fileName.substring(0, fileName.length() - 3) + "jpg";
            }
     ...
     * @throws Exception
     */
    private void opeMediumandSmall(String Path, int width, int height,
            Boolean synFlag, int siteId, int picId) throws Exception {
        logger.debug("开始处理图片");
        int picSize = width > height ? width : height;
        ...
        if (medium == null) {
            ...
            if (mediumSize <= picSize) {
                ImageAnalyseUtil.gmAlterImg(mediumSize, Path,
                        initFullPathByOrder(mediumPath, fileName), width,
                        height, synFlag);
            } else {
                ImgFileUtils.makeDirectory(mediumAllPath);
        ...
        if (big == null) {
            int bigSize = Integer.valueOf(sysConfigService
                    .getDbSysConfig(SysConfigConstant.BIG_PIC_SIZE, siteId));
            String bigPath = sysConfigService
                    .getDbSysConfig(SysConfigConstant.BIG_PIC_PATH, siteId);
            String bigAllPath = initFullPathByOrder(bigPath, fileName);
            if (bigSize <= picSize) {
                ImageAnalyseUtil.gmAlterImg(bigSize, Path,
                        initFullPathByOrder(bigPath, fileName), width, height,
                        synFlag);
            } else {
        ...
        if (wmM == null) {
            ...
            String watermarkedmedium = sysConfigService.getDbSysConfig(
                    SysConfigConstant.WATERMARKEDMEDIUM_PIC_PATH, siteId);
            String wmAllPath = initFullPathByOrder(watermarkedmedium, fileName);
            ImageAnalyseUtil.waterMarkPic(wmAllPath, mediumAllPath, waterPic,
                    position, true);
...
//        List<String> showPicByGroupId();
        PictureService service = new PictureService();
        List<CpPicture> list = service.showPicByGroupId(7242);
        for(CpPicture c : list){
