...
public class PictureDataExchangeService {
	 ...
	 * @author xiayunan
	 */
	public   List<CpPicture> uploadMorePicByPicFiles(File dir,int siteId,Map<String,String> map) throws Exception{
...
//		}
	 ...
	 * @author xiayunan
	 */
	public CpPicture uploadOnePicByPicFile(File picFile,String filename,int siteId,Map<String,String> map) throws Exception{
		//原图存放路径
...
//		pic.setSourcePictureName(picFile.getOriginalFilename());
		...
		try{
			...
			pic=loadInfoByIPTCForDataExchange(pic, oriFile,map);
...
//			throw new Exception("上传图片包含非IPTC图片，请重新选择图片上传");
	 ...
	 * @throws Exception
	 */
	private CpPicture loadInfoByIPTCForDataExchange(CpPicture ap, File temp,Map<String,String> map) throws Exception {
		try {
			Directory IPTCInfo = ImageAnalyseUtil.extratIPTCFromFile(temp
					.getAbsolutePath());
			Boolean isnull = true;
